...
test('puzzle sums share a stable key and reveal the same picture', async ({ page }) => {
  ...
  for (const problem of problems) {
    ...
    expect(key.some(entry => entry.results.includes(problem.split('+').reduce((sum, part) => sum + Number(part), 0)))).toBe(true)
  }
  ...
  problems.forEach((problem, index) => {
    ...
    expect(colors[index]).toBe(key.find(entry => entry.results.includes(sum))?.color)
  })
