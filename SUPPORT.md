...
| Large white margins leave the subject too small | Enable **Trim background margins** beside the upload. Preview the crop before generating; Auto size follows its proportions. |
| Trimming leaves margins or removes pale detail | All pixels below the near-white threshold contribute to the bounds, including stray specks. Very pale details may count as background. Manual grids can also add aspect-fit padding after cropping; disable trimming to restore the original. |
