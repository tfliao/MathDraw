...
export function Worksheet({ puzzle, mode }: { puzzle: Puzzle; mode: ViewMode }) {
  ...
  return (
    <article className={`worksheet ${mode}-sheet`} style={style} aria-label={mode === 'puzzle' ? t.puzzleWorksheet : t.answerWorksheet}>
      ...
      <p className="worksheet-instructions">{mode === 'puzzle' ? `${additionOnly ? t.sumInstructions : t.mathInstructions}${puzzle.settings.operators.includes('*') ? ` ${t.multiplyHelp}` : ''}` : t.answerInstructions}</p>
      ...
      <table className="color-key" aria-label={t.sharedKey}>
