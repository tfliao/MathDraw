...
describe('readable worksheet sizing', () => {
  ...
  it('expands cells for long problems even when there are only four columns', () => {
    const puzzle = createPuzzle({ rows: 24, columns: 4, palette: [[0, 0, 0]], assignments: Array(96).fill(0) }, { ...DEFAULT_SETTINGS, maxOperand: 99, operators: ['*'] }, () => 1 - Number.EPSILON)
  ...
  it('allows space for sixteen colors and three answers per key entry', () => {
    ...
    const puzzle = createPuzzle({ rows: 24, columns: 64, palette, assignments: Array.from({ length: 1536 }, (_, n) => n % 16) }, { ...DEFAULT_SETTINGS, maxOperand: 99, operators: ['*'], maxColors: 16, multiMap: true }, () => .999)
    expect(worksheetLayout(puzzle)).toEqual({
